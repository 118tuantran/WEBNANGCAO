...
public sealed class InventoryService(WarehouseDbContext db)
{
    public async Task<Receipt> CreateReceiptAsync(int warehouseId, int userId, IReadOnlyCollection<MovementLine> lines)
    {
        ...
        {
            Number = $"GR-{DateTime.UtcNow:yyyyMMddHHmmssfff}",
            WarehouseId = warehouseId,
            CreatedBy = userId,
            Lines = lines.Select(x => new ReceiptLine { ProductId = x.ProductId, Quantity = x.Quantity }).ToList()
        };
    ...
    public async Task<Issue> CreateIssueAsync(int warehouseId, int userId, IReadOnlyCollection<MovementLine> lines)
    {
        ...
        var issue = new Issue
        {
            Number = $"GI-{DateTime.UtcNow:yyyyMMddHHmmssfff}",
            WarehouseId = warehouseId,
            CreatedBy = userId,
            Lines = lines.Select(x => new IssueLine { ProductId = x.ProductId, Quantity = x.Quantity }).ToList()
        };
    ...
    public async Task ApproveReceiptAsync(int id, int approverId)
    {
        ...
        var inventoryMap = await GetInventoryMapAsync(receipt.WarehouseId, receipt.Lines.Select(x => x.ProductId));
        foreach (var line in receipt.Lines)
        {
            var inventory = GetInventoryForMovement(inventoryMap, receipt.WarehouseId, line.ProductId);
            var before = inventory.Quantity;
            var after = before + line.Quantity;
            if (after < 0)
                throw new InvalidOperationException("Không đủ tồn khả dụng để duyệt phiếu xuất.");
    ...
    public async Task ApproveIssueAsync(int id, int approverId)
    {
        ...
        var inventoryMap = await GetInventoryMapAsync(issue.WarehouseId, issue.Lines.Select(x => x.ProductId));
        foreach (var line in issue.Lines)
        {
            var inventory = GetInventoryForMovement(inventoryMap, issue.WarehouseId, line.ProductId);
            var before = inventory.Quantity;
            var after = before - line.Quantity;
            if (after < 0)
                throw new InvalidOperationException("Không đủ tồn khả dụng để duyệt phiếu xuất.");
    ...
    public Task<List<InventoryView>> GetInventoryAsync(int? warehouseId, string? sku)
    {
        ...
        return query.OrderBy(x => x.Product!.Sku)
            .Select(x => new InventoryView(x.WarehouseId, x.Product!.Sku, x.Product.Name, x.Quantity, x.MinStock, x.Quantity <= x.MinStock))
            .ToListAsync();
    }
    ...
    public Task<List<StockTransaction>> GetTransactionsAsync(int? warehouseId, int? productId) =>
        db.StockTransactions.AsNoTracking()
            ...
            .Where(x => !productId.HasValue || x.ProductId == productId.Value)
    ...
    public async Task<Stocktake> CreateStocktakeAsync(int warehouseId, int userId, IReadOnlyCollection<int>? productIds)
    {
        ...
        var ids = productIds is { Count: > 0 }
            ? productIds.Distinct().ToArray()
            : await db.Inventories.Where(x => x.WarehouseId == warehouseId).Select(x => x.ProductId).Distinct().ToListAsync();
        ...
        {
            Number = $"ST-{DateTime.UtcNow:yyyyMMddHHmmssfff}",
            WarehouseId = warehouseId,
            CreatedBy = userId,
            Details = inventories.Select(x => new StocktakeDetail { ProductId = x.ProductId, SystemQuantity = x.Quantity }).ToList()
        };
    ...
    public async Task<StocktakeDetail> RecordCountAsync(int stocktakeId, int detailId, int actualQuantity, string? reason)
    {
        ...
        var detail = await db.StocktakeDetails.Include(x => x.Stocktake)
            .SingleOrDefaultAsync(x => x.Id == detailId && x.StocktakeId == stocktakeId)
    ...
    public async Task ApproveStocktakeAsync(int stocktakeId, int approverId)
    {
        ...
        var inventoryMap = await GetInventoryMapAsync(stocktake.WarehouseId, stocktake.Details.Select(x => x.ProductId));
        foreach (var detail in stocktake.Details)
        {
            ...
            var inventory = GetInventoryForMovement(inventoryMap, stocktake.WarehouseId, detail.ProductId);
            var before = inventory.Quantity;
            var after = before + detail.Difference!.Value;
            ...
            db.StockTransactions.Add(new StockTransaction
            {
                ...
                BeforeQuantity = before,
                ChangeQuantity = detail.Difference.Value,
                AfterQuantity = after,
    ...
    public async Task<List<ReportRow>> GetReportAsync(DateTime? from, DateTime? to, int? warehouseId)
    {
        ...
        var movements = await inPeriod.GroupBy(x => x.ProductId).Select(x => new
        {
            ProductId = x.Key,
            Receipt = x.Where(y => y.Type == MovementType.Receipt).Sum(y => y.ChangeQuantity),
            Issue = -x.Where(y => y.Type == MovementType.Issue).Sum(y => y.ChangeQuantity),
            Adjustment = x.Where(y => y.Type == MovementType.Adjustment).Sum(y => y.ChangeQuantity)
        }).ToListAsync();
        ...
        return inventory.GroupJoin(movements, x => x.ProductId, x => x.ProductId, (item, changes) =>
        {
            var c = changes.SingleOrDefault();
            var receipt = c?.Receipt ?? 0;
            var issue = c?.Issue ?? 0;
            var adjustment = c?.Adjustment ?? 0;
            return new ReportRow(item.Sku, item.ProductName, item.Quantity - receipt + issue - adjustment, receipt, issue, adjustment, item.Quantity);
        }).OrderBy(x => x.Sku).ToList();
    }

    private async Task<Dictionary<int, Inventory>> GetInventoryMapAsync(int warehouseId, IEnumerable<int> productIds)
    {
        var ids = productIds.Distinct().ToArray();
        if (ids.Length == 0)
            return new Dictionary<int, Inventory>();

        var inventories = await db.Inventories
            .Where(x => x.WarehouseId == warehouseId && ids.Contains(x.ProductId))
            .ToListAsync();

        var missing = ids.Where(id => inventories.All(x => x.ProductId != id)).ToArray();
        if (missing.Length > 0)
            throw new InvalidOperationException("SKU chưa được cấu hình trong kho.");

        return inventories.ToDictionary(x => x.ProductId);
    }

    private static Inventory GetInventoryForMovement(Dictionary<int, Inventory> inventoryMap, int warehouseId, int productId)
    {
        if (!inventoryMap.TryGetValue(productId, out var inventory))
            throw new InvalidOperationException($"SKU {productId} chưa được cấu hình trong kho {warehouseId}.");
        return inventory;
    }
    ...
    private async Task EnsureProductsAsync(IEnumerable<MovementLine> lines)
    {
        ...
        if (ids.Length == 0)
            throw new InvalidOperationException("Danh sách SKU không được rỗng và số lượng phải lớn hơn 0.");

        if (await db.Products.CountAsync(x => ids.Contains(x.Id) && x.IsActive) != ids.Length)
            throw new InvalidOperationException("SKU không tồn tại hoặc đã bị khóa.");
    }

    private async Task EnsureActiveWarehouseAsync(int id)
    {
        if (!await db.Warehouses.AnyAsync(x => x.Id == id && x.IsActive))
            throw new InvalidOperationException("Kho không tồn tại hoặc đã bị khóa.");
    }

    private static void ValidateLines(IReadOnlyCollection<MovementLine> lines)
    {
        if (lines.Count == 0 || lines.Any(x => x.Quantity <= 0))
            throw new InvalidOperationException("Danh sách SKU không được rỗng và số lượng phải lớn hơn 0.");
    }

    private static void EnsurePending(DocumentStatus status)
    {
        if (status != DocumentStatus.Pending)
            throw new InvalidOperationException("Chứng từ không ở trạng thái chờ duyệt.");
    }

    private static void EnsureNotSelfApproval(int creatorId, int approverId)
    {
        if (creatorId == approverId)
            throw new InvalidOperationException("Người tạo không được tự duyệt chứng từ.");
    }
}
