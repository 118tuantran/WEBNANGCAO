...
public sealed class ReportController(InventoryService service) : ControllerBase
{
    ...
    public async Task<IActionResult> InOutStock([FromQuery] DateTime? from, [FromQuery] DateTime? to, [FromQuery] int? warehouseId)
    {
        ...
        return Ok(await service.GetReportAsync(from, to, warehouseId));
    }
}
